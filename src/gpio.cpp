...
struct Gpio::Line
{
  ...
  Line(const std::string& gpioDevice, const std::string& consumerName, int line, LineMode mode, LineBias bias = LineBias::Off, int gpioFd = -1):
    ...
    fd_(-1)
  {
    ...
    strncpy(lineReq.consumer, consumerName.c_str(), GPIO_MAX_NAME_SIZE);
...
Gpio::Gpio(const std::string& gpioDevice, const std::string& consumerName):
  stopThread_(false),
  gpioDevice_(gpioDevice),
...
void Gpio::setupLine(int line, LineMode mode, LineBias bias)
{
  ...
  {
    lines_.emplace(line, std::make_unique<Line>(gpioDevice_, consumerName_, line, mode, bias));
    setupPollfd();
  }
  else
  {
